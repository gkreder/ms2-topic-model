...
def add_fd(form1, fd2):
	fd1 = {x[0] : x[1] for x in molmass.Formula(form1).composition().astuple()}
...
def sub_formulas(form1, form2, mode = 'report'): # form1 - form2
	fd1 = {x[0] : x[1] for x in molmass.Formula(form1).composition().astuple()}
	fd2 = {x[0] : x[1] for x in molmass.Formula(form2).composition().astuple()}
	for sub_element, sub_num in fd2.items():
		if sub_element not in fd1.keys():
			fd1[sub_element] = 0
		fd1[sub_element] -= sub_num
...
def findBestForms(mass, allForms, toleranceDa = 0.005, charge = 0, verbose = False):
    if charge != 0:
...
if __name__ == "__main__":
	...
	for s in tqdm(mgf.MGF(args.in_mgf), total = num_spectra):
		...
		allForms = generateAllForms(form)
		def get_output(i_mz, mz):        
			formOutput = findBestForms(mz, allForms, toleranceDa = 0.1)
			if formOutput[0][0] == None:
				a = None
				tm = None
				o = None
			else:
